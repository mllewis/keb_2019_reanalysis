...
```{r}
LANG_ANIMAL_DISTANCE <- here("data/processed/animal_texture_distances_language_wiki.csv")
...
```{r}
lm(human_similarity_skin~ language_similarity_simple_dist*participant_type ,  data = full_sim_data_wide2 %>% mutate_if(is.numeric, scale)) %>%
  summary()

lm(human_similarity_skin~ language_similarity_simple_dist*participant_type + 
