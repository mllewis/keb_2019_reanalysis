...
### correct - mean of incorrect (for human and language)
```{r, fig.width =10}
correct_language_data_diff <- language_data %>%
  select(-group)  %>%
  right_join(human_data %>% distinct(animal, texture, correct)) %>%
  rename(language_similarity = similarity) %>%
  group_by(set_id, animal, correct) %>%
  summarize(language_similarity = mean(language_similarity)) %>%
  ungroup() %>%
  spread(correct, language_similarity) %>%
  mutate(language_similarity = `1` - `0`) %>%
  select(set_id, animal, language_similarity) %>%
  right_join(correct_human_data %>% distinct(animal, texture))
...
correct_human_data_diff <- human_data %>%
  ...
  select(group, animal, similarity) %>%
  right_join(correct_human_data %>% distinct(animal, texture))
