...
For consistency, taxonomic distances are reported here in terms of similariry (1 - evolutionary distance).
...
Notably, in a regression predicting human similarity with both taxonomic and linguistic similarity, both measures predict indepndent variance in human judgements for all three dimensions. 

### Color
```{r}
lm_df <- full_sim_data_wide2 %>%
  left_join(taxonomic_long %>% rename("taxonomic_sim" = similarity ) %>% select(-sim_type)) %>%
...
In this analyis (corresponding to Fig. 1b in the Main Text), we used language data to predict human responses from a task where participants had to indciate whether each of the 30 animals had a texture of either fur, feathers, scales, or skin. 
...
Correlations with with "blind" excluded: 
```{r}
all_data %>%
  filter(is_blink == "nb") %>%
  rename(participant_group = group_type) %>%
...
de Vries, Andrie  &  Ripley, Brian D. (2016). ggdendro: Create Dendrograms and Tree Diagrams Using 'ggplot2'. R package version 0.1-20. https://CRAN.R-project.org/package=ggdendro
