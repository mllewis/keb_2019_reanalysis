...
cor_df <- color_cors %>%
 ...
 ggplot(cor_df, aes(x = fct_rev(participant_type), y = z, fill = participant_type)) +
...
b_vs_s_cor_all <- cor.test(b_vs_s_all$Blind, b_vs_s_all$Sighted, method = "spearman") %>%
  tidy() %>%
  mutate_if(is.numeric, round, 2)
```

Including all items, the correlation between blind and sighted participants is $\rho$ = `r pull(b_vs_s_cor_all, estimate)` (_p_ < .01).
