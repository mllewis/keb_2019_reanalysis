...
# color
full_sim_data_wide <-  full_join(language_data, human_data,
                                  by = c("animal1", "animal2"))    %>%
  spread(similarity_type, human_similarity) %>%
  filter(animal1 < animal2)

color_cors <- full_sim_data_wide %>%
...
# texture
texture_cors <- full_sim_data_wide %>%
  group_by(participant_type) %>%
  nest() %>%
...
cor_df <- color_cors %>%
  ...
  rename(rho = estimate) %>%
  mutate(z = fisherz(rho), # z-transform
         se = 1/sqrt(n - 3), #https://en.wikipedia.org/wiki/Fisher_transformation
         estimate_se_l = z - se,
         estimate_se_h = z + se,
...
fig_a <- ggplot(cor_df, aes(x = fct_rev(participant_type), y = z, fill = participant_type)) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_grid(~dimension, drop = T, scales="free_x",space = "free_x") +
  xlab("Language as predictor of...") +
  geom_text(aes(y = z + .07, label = sig), size = 6) +
